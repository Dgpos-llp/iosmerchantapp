...
class _AllTaxwiseSalesReportPageState extends State<AllTaxwiseSalesReportPage> {
  ...
  final List<_Col> _fixedColumns = [
    ...
    const _Col('Taxable Amount', 'taxableAmount'),
  ];
  ...
  void _calculateTotals(List<_TaxwiseBillRow> rows) {
    ...
    _totals = {
      'restaurant': 'Total',
      // Formatting total taxable amount to 3 decimal places
      'taxableAmount': totalTaxable.toStringAsFixed(3),
    };

    // Formatting each dynamic tax total to 3 decimal places
  ...
  Future<void> _exportExcel() async {
    ...
    final totalData = _visibleColumns.map((c) => _totals[c.key] ?? '').toList();
    ...
    if (kIsWeb) {
      web_exporter.saveFileWeb(fileBytes!, 'AllTaxwiseSalesReport.xlsx');
      ...
      final String path = '${Directory.current.path}/TaxwiseWise.xlsx';
                                ...
                                return Row(
                                  children: _visibleColumns.map((col) {
                                    return Container(
                                      ...
                                      child: Text(row.getField(col.key)),
                                    );
                                  }).toList(),
                                );
                              },
      ...
      child: Row(
        children: _visibleColumns.map((col) {
          ...
          if (col.key == 'restaurant') {
            ...
            displayValue = "";
          } else if (col.key == 'taxableAmount') {
            displayValue = _totals['taxableAmount']?.toString() ?? '0.000';
          } else {
  ...
  String getField(String key) {
    ...
    switch (key) {
      ...
      case 'billDate':
        return billDate;
      case 'taxableAmount':
        return format3(taxableAmount);
      default:
        return format3(taxAmounts[key]?.toString());
    }
  }
}