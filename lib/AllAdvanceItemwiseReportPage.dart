...
class AllAdvanceItemwiseReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllAdvanceItemwiseReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllAdvanceItemwiseReportPage> createState() => _AllAdvanceItemwiseReportPageState();
}

class _AllAdvanceItemwiseReportPageState extends State<AllAdvanceItemwiseReportPage> {
  ...
  final List<_Col> _baseColumns = [
    ...
    const _Col('Adv Order ID', 'advanceOrderId'),
    const _Col('Customer', 'customerName'),
    const _Col('Mobile', 'customerMobile'),
    const _Col('Item', 'itemName'),
    const _Col('Item Code', 'item'),
    const _Col('Qty', 'quantity'),
    const _Col('Wt', 'weight'),
    const _Col('Rate', 'rate'),
    const _Col('Amt', 'amount'),
    const _Col('Modifiers', 'modifiers'),
    const _Col('Mod Qty', 'modifierQty'),
    const _Col('Mod Price', 'modifierPrice'),
  ];
  ...
  void initState() {
    super.initState();
    _allColumns = List.from(_baseColumns);
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<AdvanceItemwiseReport>> dbToAdvanceItemwise =
    await UserData.fetchAdvanceItemwiseForDbs(config, dbList, startDate, endDate);

    for (final dbKey in dbToAdvanceItemwise.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToAdvanceItemwise[dbKey]!) {
        Map<String, dynamic> row = {
          'restaurant': brand,
          'advanceOrderId': report.advanceOrderId,
          ...
          'modifierQty': report.modifierQuantities,
          'modifierPrice': report.modifierPrices,
        };
        _allRows.add(row);

        // Calculate totals
        totalQuantity += int.tryParse(report.quantity) ?? 0;
        totalAmount += double.tryParse(report.amount) ?? 0.0;
      }
    }
  ...
  Future<void> _exportExcel() async {
    try {
      ...
      for (int i = 0; i < _visibleColumns.length; i++) {
        ...
        else if (colKey == 'quantity') value = totalQuantity.toString();
        else if (colKey == 'amount') value = _format3(totalAmount);
      ...
      if (kIsWeb) {
        web_exporter.saveFileWeb(fileBytes!, 'AdvanceItemwiseReport.xlsx');
        if (mounted) {
        ...
        final String path = '${directory.path}/AdvanceItemwiseReport_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Advance Itemwise", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: const Text("Advance Items", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
        ),
        Expanded(
          child: Scrollbar(
    ...
    return Container(
      ...
      child: Row(
        children: _visibleColumns.map((col) {
          ...
          else if (col.key == 'quantity') text = totalQuantity.toString();
          else if (col.key == 'amount') text = _format3(totalAmount);
