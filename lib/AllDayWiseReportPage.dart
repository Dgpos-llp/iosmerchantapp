...
class AllDayWiseReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllDayWiseReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllDayWiseReportPage> createState() => _AllDayWiseReportPageState();
}

class _AllDayWiseReportPageState extends State<AllDayWiseReportPage> {
  ...
  final List<String> _numericKeys = [
    'subtotal', 'billDiscount', 'netTotal', 'billTax', 'billTotal', 'noOfBills'
  ];
  ...
  final List<_Col> _allColumns = [
    ...
    const _Col('Disc', 'billDiscount'),
    const _Col('Net', 'netTotal'),
    const _Col('Tax', 'billTax'),
    const _Col('Grand', 'billTotal'),
    const _Col('Bills', 'noOfBills'),
  ];
  late List<_Col> _visibleColumns;
  List<_DayWiseRow> _allRows = [];
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<DayWiseReport>> dbToDayWise =
    await UserData.fetchDayWiseForDbs(config, dbList, startDate, endDate);

    for (final dbKey in dbToDayWise.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToDayWise[dbKey]!) {
        _allRows.add(_DayWiseRow(
          restaurant: brand,
          report: report,
          formatter: _format3,
        ));
        ...
        totals['billDiscount'] = totals['billDiscount']! + (double.tryParse(report.billDiscount) ?? 0.0);
        totals['netTotal'] = totals['netTotal']! + (double.tryParse(report.netTotal) ?? 0.0);
        totals['billTax'] = totals['billTax']! + (double.tryParse(report.billTax) ?? 0.0);
        totals['billTotal'] = totals['billTotal']! + (double.tryParse(report.billTotal) ?? 0.0);
        totals['noOfBills'] = totals['noOfBills']! + (double.tryParse(report.noOfBills) ?? 0.0);
      }
    }
  ...
  _DayWiseRow get totalRow {
    return _DayWiseRow(
      restaurant: "Total",
      report: DayWiseReport(
        billDate: "",
        subtotal: _format3(totals['subtotal']),
        billDiscount: _format3(totals['billDiscount']),
        netTotal: _format3(totals['netTotal']),
        billTax: _format3(totals['billTax']),
        billTotal: _format3(totals['billTotal']),
  ...
  Future<void> _exportExcel() async {
    try {
      ...
      final sheet = excelFile['Day Wise Report'];
      ...
      if (kIsWeb) {
        web_exporter.saveFileWeb(fileBytes!, 'DayWiseReport.xlsx');
        if (mounted) {
        ...
        final String path = '${directory.path}/DayWiseReport_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Day Wise", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: const Text("Day Wise Summary", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
        ),
        Expanded(
          child: Scrollbar(
...
class _DayWiseRow {
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'subtotal': return report.subtotal;
      case 'billDiscount': return report.billDiscount;
      case 'netTotal': return report.netTotal;
      case 'billTax': return report.billTax;
      case 'billTotal': return report.billTotal;
      case 'noOfBills': return report.noOfBills;
      default: return '';
    }
  }
}
