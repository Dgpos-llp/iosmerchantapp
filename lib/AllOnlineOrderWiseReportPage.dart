...
class _AllOnlineDaywiseReportPageState extends State<AllOnlineDaywiseReportPage> {
    ...
    if (selectedDbKey == null || selectedDbKey == "All") {
      for (final db in dbToOnlineDaywise.keys) {
        ...
        for (final report in dbToOnlineDaywise[db]!) {
          _allRows.add(_OnlineDaywiseRow.fromReport(
            report: report,
            restaurant: brand,
          ));
        }
      ...
      for (final report in dbToOnlineDaywise[selectedDbKey!] ?? []) {
        _allRows.add(_OnlineDaywiseRow.fromReport(
          report: report,
          restaurant: brand,
        ));
  ...
  _OnlineDaywiseRow get totalRow {
    ...
    // Formatting totals to 3 decimal places
  ...
  Future<void> _exportExcel() async {
    ...
    reportCell.value = "All Online Order Wise Report";
    ...
    if (kIsWeb) {
      // WEB PLATFORM
      web_exporter.saveFileWeb(fileBytes!, 'AllOnlineOrderWiseReport.xlsx');
      ...
      final String path = '${Directory.current.path}/AllOnlineDaywiseReport.xlsx';
...
class _OnlineDaywiseRow {
  ...
  factory _OnlineDaywiseRow.fromReport({required OnlineDaywiseReport report, required String restaurant}) {
    ...
    return _OnlineDaywiseRow(
      ...
      subtotal: format3(report.subtotal),
      discount: format3(report.discount),
      packagingCharge: format3(report.packagingCharge),
      deliveryCharge: format3(report.deliveryCharge),
      tax: format3(report.tax),
      total: format3(report.total),
