...
class AllCreditReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllCreditReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllCreditReportPage> createState() => _AllCreditReportPageState();
}

class _AllCreditReportPageState extends State<AllCreditReportPage> {
  ...
  final List<_Col> _allColumns = [
    ...
    const _Col('Bill Amt', 'billAmount'),
    const _Col('Credit Amt', 'creditAmount'),
  ];
  late List<_Col> _visibleColumns;
  List<_CreditRow> _allRows = [];
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<CreditReport>> dbToCreditReport =
    await UserData.fetchCreditReportForDbs(config, dbList, startDate, endDate);

    for (final dbKey in dbToCreditReport.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToCreditReport[dbKey]!) {
        _allRows.add(_CreditRow(
          restaurant: brand,
          report: report,
          formatter: _format3,
        ));
        ...
        // Calculate totals
        totals['billAmount'] = totals['billAmount']! + (double.tryParse(report.billAmount) ?? 0.0);
        totals['creditAmount'] = totals['creditAmount']! + (double.tryParse(report.creditAmount) ?? 0.0);
      }
    }
  ...
  _CreditRow get totalRow {
    return _CreditRow(
      ...
      report: CreditReport(
        ...
        billNo: "",
        billDate: "",
        billAmount: _format3(totals['billAmount']),
        creditAmount: _format3(totals['creditAmount']),
      ),
      formatter: _format3,
    );
  }
  ...
  Future<void> _exportExcel() async {
    try {
      ...
      final sheet = excelFile['Credit Report'];
      ...
      if (kIsWeb) {
        web_exporter.saveFileWeb(fileBytes!, 'CreditReport.xlsx');
        if (mounted) {
        ...
        final String path = '${directory.path}/CreditReport_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Credit Sales", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(bool isMobile) {
    double colWidth = isMobile ? 120.0 : 150.0;
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: const Text("Credit Sales Details", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
        ),
        Expanded(
          child: Scrollbar(
...
class _CreditRow {
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'customerName': return report.customerName;
      case 'billNo': return report.billNo;
      case 'billDate': return report.billDate;
      case 'billAmount': return report.billAmount;
      case 'creditAmount': return report.creditAmount;
      default: return '';
    }
  }
}
