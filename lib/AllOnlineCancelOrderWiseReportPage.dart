...
class _AllOnlineCancelOrderWiseReportPageState extends State<AllOnlineCancelOrderWiseReportPage> {
  ...
  final List<_Col> _allColumns = [
    const _Col('Restaurant', 'restaurantName'),
    const _Col('Order Source', 'orderFrom'),
    const _Col('Order No.', 'onlineOrderId'),
    const _Col('Item Name', 'itemName'),
    const _Col('Quantity', 'quantity'),
    const _Col('Unit Price', 'unitPrice'),
    const _Col('Item Gross Total', 'itemGrossTotal'),
    const _Col('Order Gross Total', 'orderGrossTotal'),
    const _Col('Item Total Amount', 'totalAmount'),
  ];
  ...
  Future<void> _fetchData() async {
    ...
    for (final dbKey in dbToOnlineCancelOrderwise.keys) {
      final brandName = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToOnlineCancelOrderwise[dbKey]!) {
  ...
  _OnlineCancelOrderRow get totalRow {
    ...
    return _OnlineCancelOrderRow(
      ...
      onlineOrderId: "",
      itemName: "",
      quantity: sum((r) => r.quantity).toStringAsFixed(3),
      unitPrice: "",
      itemGrossTotal: sum((r) => r.itemGrossTotal).toStringAsFixed(3),
      orderGrossTotal: sum((r) => r.orderGrossTotal).toStringAsFixed(3),
      totalAmount: sum((r) => r.totalAmount).toStringAsFixed(3),
    );
  }
  ...
  Future<void> _exportExcel() async {
    ...
    sheet.cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: rowNum))
      ..value = "Online Cancel Order Report"
      ..cellStyle = boldStyle;
    rowNum += 2;
    ...
    if (kIsWeb) {
      web_exporter.saveFileWeb(fileBytes!, 'OnlineCancelOrderReport.xlsx');
    } else {
      final String path = '${Directory.current.path}/OnlineCancelOrder.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("Online Cancel Order Report", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Online Cancel Order", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(double screenWidth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: const Text("Order Cancellation Details", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))),
      Expanded(
        child: Scrollbar(
...
class _OnlineCancelOrderRow {
  final String restaurantName, orderFrom, onlineOrderId, itemName, quantity, unitPrice, itemGrossTotal, orderGrossTotal, totalAmount;
  _OnlineCancelOrderRow({required this.restaurantName, required this.orderFrom, required this.onlineOrderId, required this.itemName, required this.quantity, required this.unitPrice, required this.itemGrossTotal, required this.orderGrossTotal, required this.totalAmount});
  ...
  factory _OnlineCancelOrderRow.fromReport({required OnlineCancelOrderReport report, required String restaurant}) {
    ...
    return _OnlineCancelOrderRow(restaurantName: restaurant, orderFrom: report.orderFrom, onlineOrderId: report.onlineOrderId, itemName: report.itemName, quantity: f3(report.quantity), unitPrice: f3(report.unitPrice), itemGrossTotal: f3(report.itemGrossTotal), orderGrossTotal: f3(report.orderGrossTotal), totalAmount: f3(report.totalAmount));
  }

  dynamic getField(String k) {
    switch (k) {
      case 'restaurantName': return restaurantName; case 'orderFrom': return orderFrom; case 'onlineOrderId': return onlineOrderId; case 'itemName': return itemName; case 'quantity': return quantity; case 'unitPrice': return unitPrice; case 'itemGrossTotal': return itemGrossTotal; case 'orderGrossTotal': return orderGrossTotal; case 'totalAmount': return totalAmount; default: return '';
    }
  }
}
