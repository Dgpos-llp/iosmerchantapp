...
class _AllCancelBillReportPageState extends State<AllCancelBillReportPage> {
  ...
  // List of numeric keys to handle alignment globally
  final List<String> _numericKeys = ['grandTotal', 'netTotal'];

  final List<_Col> _allColumns = [
    const _Col('Restaurant', 'restaurant'),
    const _Col('Bill No.', 'billNo'),
    const _Col('Bill Date', 'billDate'),
    const _Col('Cancel Date', 'cancelDate'),
    const _Col('Created Time', 'createdTime'),
    const _Col('Cancel Time', 'cancelTime'),
    const _Col('Created User', 'createdUser'),
    const _Col('Cancel User', 'cancelUser'),
    const _Col('Bill Type', 'billType'),
    const _Col('KOT(s)', 'kot'),
    const _Col('Item(s)', 'item'),
    const _Col('Taxes', 'taxes'),
    ...
    for (final billNo in billGroups.keys) {
      ...
      for (final report in group) {
        if (report.items != null && report.items.trim().isNotEmpty) {
          itemsLines += report.items.split(';').map((e) => e.trim()).where((e) => e.isNotEmpty).join('\n') + '\n';
        }
        ...
        if (report.taxes != null && report.taxes.trim().isNotEmpty) {
          for (final tax in report.taxes.split(';')) {
            ...
            if (parts.length == 2) {
              ...
              final val = double.tryParse(parts[1].replaceAll(",", "").trim()) ?? 0.0;
      ...
      String taxesLine = taxMap.entries.map((e) => "${e.key}: ${e.value.toStringAsFixed(3)}").join("; ");
  ...
  _CancelBillRow get totalRow {
    double sum(String Function(_CancelBillRow) getter) {
      return _allRows.fold(0.0, (a, b) {
        final value = double.tryParse(getter(b).replaceAll(',', ''));
        return a + (value ?? 0.0);
      });
    }

    return _CancelBillRow(
      restaurant: "Total", billNo: "", billDate: "", cancelDate: "",
      createdTime: "", cancelTime: "", createdUser: "", cancelUser: "",
      billType: "", kot: "", item: "", taxes: "",
      grandTotal: sum((r) => r.grandTotal).toStringAsFixed(3),
      netTotal: sum((r) => r.netTotal).toStringAsFixed(3),
      formatter: _format3,
    );
  }
  ...
  Future<void> _exportExcel() async {
    ...
    if (kIsWeb) {
      web_exporter.saveFileWeb(fileBytes!, 'CancellationReport.xlsx');
    } else {
      final String path = '${Directory.current.path}/CancellationReport.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("Cancellation Report", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Cancellation", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(double screenWidth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: const Text("Cancelled Bill Logs", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))),
      Expanded(
        child: Scrollbar(
    ...
    return Container(
      ...
      child: Row(
        children: _visibleColumns.map((col) {
          // Check if column is numeric to align header right
          bool isNumeric = _numericKeys.contains(col.key);
          return Container(
            width: w, height: h,
            alignment: isNumeric ? Alignment.centerRight : Alignment.centerLeft,
  ...
  Widget _buildTotalRow(double h, double w) {
    return Container(
      ...
      child: Row(children: _visibleColumns.map((col) {
        bool isNumeric = _numericKeys.contains(col.key);
        return Container(width: w, height: h, alignment: isNumeric ? Alignment.centerRight : Alignment.centerLeft, padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(border: Border(right: BorderSide(color: Colors.grey.shade300))), child: Text(totalRow.getField(col.key).toString(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF4154F1))));
      }).toList()),
    );
  }
}
...
class _CancelBillRow {
  final String restaurant, billNo, billDate, cancelDate, createdTime, cancelTime, createdUser, cancelUser, billType, kot, item, taxes, grandTotal, netTotal;
  final String Function(dynamic) formatter;

  _CancelBillRow({required this.restaurant, required this.billNo, required this.billDate, required this.cancelDate, required this.createdTime, required this.cancelTime, required this.createdUser, required this.cancelUser, required this.billType, required this.kot, required this.item, required this.taxes, required this.grandTotal, required this.netTotal, required this.formatter});
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'cancelDate': return cancelDate;
      case 'createdTime': return createdTime;
      case 'cancelTime': return cancelTime;
      case 'createdUser': return createdUser;
      case 'cancelUser': return cancelUser;
      case 'billType': return billType;
      case 'kot': return kot;
      case 'item': return item;
      case 'taxes': return taxes;
      case 'grandTotal': return formatter(grandTotal);
      case 'netTotal': return formatter(netTotal);
      default: return '';
    }
  }
}
