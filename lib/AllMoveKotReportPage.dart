...
class _AllMoveKotReportPageState extends State<AllMoveKotReportPage> {
  ...
  Future<void> _fetchData() async {
    ...
    if (selectedDbKey == null || selectedDbKey == "All") {
      final List<MoveKotReport> allItems =
      dbToMoveKot.values.expand((x) => x).toList();
      for (final item in allItems) {
        _allRows.add(_MoveKotRow(restaurant: "ALL", report: item));
      }
    } else {
      for (final list in dbToMoveKot.values) {
        for (final item in list) {
          _allRows.add(_MoveKotRow(
  ...
  _MoveKotRow get totalRow {
    ...
    return _MoveKotRow(
      ...
      report: MoveKotReport(
        kotId: "",
        // Total Qty is handled as double in the model, formatted to 3 decimals in getField
        qty: sum((r) => r.report.qty),
        reason: "",
        user: "",
        cancelDate: "",
        // Rate is a string in the model, parse and sum, then format to 3 decimals
        rate: sum((r) => double.tryParse(r.report.rate) ?? 0.0)
            .toStringAsFixed(3),
        waiterName: "",
        product: "",
      ),
  ...
  Future<void> _exportExcel() async {
    ...
    reportCell.value = "All Move KOT Report";
    ...
    sheet.appendRow([
      "Date From",
      _apiDateFormat.format(_startDate),
      "Date To",
      _apiDateFormat.format(_endDate)
    ]);
    rowNum += 2;
    ...
    final totalData =
    _visibleColumns.map((c) => totalRow.getField(c.key)).toList();
    ...
    if (kIsWeb) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Excel downloaded successfully')));
      }
    } else {
      // DESKTOP (Windows, Mac, Linux) AND ANDROID
      final String path = '${Directory.current.path}/AllMoveKotReport.xlsx';
                ...
                children: [
                  if (!hasOnlyOneDb)
                    Container(
                      ...
                      constraints:
                      const BoxConstraints(minWidth: 100, maxWidth: 190),
          ...
          children: [
            Container(
              ...
              padding: EdgeInsets.only(
                  left: isMobile ? 8 : 22, top: isMobile ? 10 : 18, bottom: 3),
              child: Row(
                children: [
                  Icon(Icons.home,
                  ...
                  Expanded(
                    child: Text("All Move KOT Report",
                        style: TextStyle(
                            color: Colors.red,
      ...
      child: Row(
        children: _visibleColumns.map((col) {
          return Container(
            ...
            child: Text(totalRow.getField(col.key).toString(),
                style: const TextStyle(fontWeight: FontWeight.bold)),
          );
        }).toList(),
      ),
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'kotId':
        return report.kotId;
      case 'qty':
      // Ensure quantity is formatted to 3 decimal places
        return report.qty.toStringAsFixed(3);
      case 'reason':
        return report.reason;
      case 'user':
        return report.user ?? '';
      case 'cancelDate':
        return report.cancelDate;
      case 'rate':
      // Rate is a string in the model, parse and format to 3 decimal places
        double r = double.tryParse(report.rate) ?? 0.0;
        return r.toStringAsFixed(3);
      case 'waiterName':
        return report.waiterName;
      case 'product':
                            ...
                            ConstrainedBox(
                              ...
                              child: ListView(
                                ...
                                children: widget.allColumns.map((col) {
                                  final checked =
                                  widget.visibleColumns.contains(col);
