...
class AllMoveKotReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllMoveKotReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllMoveKotReportPage> createState() => _AllMoveKotReportPageState();
}

class _AllMoveKotReportPageState extends State<AllMoveKotReportPage> {
  ...
  // Centralized list of keys that should be right-aligned (Headers, Body, and Totals)
  final List<String> _numericKeys = ['qty', 'rate'];

  final List<_Col> _allColumns = [
    const _Col('Restaurants', 'restaurant'),
    const _Col('KOT ID', 'kotId'),
    const _Col('Product', 'product'),
    const _Col('Qty', 'qty'),
    const _Col('Rate', 'rate'),
    const _Col('Reason', 'reason'),
    const _Col('User', 'user'),
    const _Col('Waiter Name', 'waiterName'),
    const _Col('Date', 'cancelDate'),
  ];
  late List<_Col> _visibleColumns;
  List<_MoveKotRow> _allRows = [];
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<MoveKotReport>> dbToMoveKot =
    await UserData.fetchMoveKotForDbs(config, dbList, startDate, endDate);

    for (final dbKey in dbToMoveKot.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToMoveKot[dbKey]!) {
        _allRows.add(_MoveKotRow(restaurant: brand, report: report));
      }
    }
    setState(() => _loading = false);
  }

  _MoveKotRow get totalRow {
    double sum(double Function(_MoveKotRow) getter) =>
        _allRows.fold(0.0, (a, b) => a + getter(b));

    return _MoveKotRow(
      restaurant: "Total",
      report: MoveKotReport(
        kotId: "",
        qty: sum((r) => r.report.qty),
        reason: "",
        user: "",
        cancelDate: "",
        rate: sum((r) => double.tryParse(r.report.rate) ?? 0.0).toStringAsFixed(3),
        waiterName: "",
        product: "",
      ),
    );
  ...
  Future<void> _exportExcel() async {
    ...
    if (kIsWeb) {
      web_exporter.saveFileWeb(fileBytes!, 'AllMoveKotReport.xlsx');
    } else {
      final String path = '${Directory.current.path}/MoveKotReport.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("All Move KOT Report", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("All Move KOT Report", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(double screenWidth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: const Text("Move Kot Report", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))),
      Expanded(
        child: Scrollbar(
...
class _MoveKotRow {
  ...
  final MoveKotReport report;

  _MoveKotRow({required this.restaurant, required this.report});

  dynamic getField(String key) {
    switch (key) {
      case 'restaurant': return restaurant;
      case 'kotId': return report.kotId;
      case 'qty': return report.qty.toStringAsFixed(3);
      case 'reason': return report.reason;
      case 'user': return report.user ?? '';
      case 'cancelDate': return report.cancelDate;
      case 'rate':
        double r = double.tryParse(report.rate) ?? 0.0;
        return r.toStringAsFixed(3);
      case 'waiterName': return report.waiterName;
