...
class _AllCancelKotReportPageState extends State<AllCancelKotReportPage> {
  ...
  // Numeric keys used for right-alignment across headers, body, and total row
  final List<String> _numericKeys = ['quantity', 'amount'];

  final List<_Col> _allColumns = [
    const _Col('Restaurant', 'restaurant'),
    const _Col('KOT ID', 'kotId'),
    const _Col('Item Names', 'itemNames'),
    const _Col('Quantity', 'quantity'),
    const _Col('Amount', 'amount'),
    const _Col('Status', 'status'),
    const _Col('Notes', 'notes'),
    const _Col('Table No.', 'tableNumber'),
    const _Col('Order Date', 'orderDate'),
    const _Col('Order Time', 'orderTime'),
  ...
  Future<void> _fetchData() async {
    ...
    for (final dbKey in dbToCancelKot.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToCancelKot[dbKey]!) {
        _allRows.add(_CancelKotRow.fromReport(report: report, restaurant: brand, formatter: _format3));
      }
    }
  ...
  _CancelKotRow get totalRow {
    double sumAmount = _allRows.fold(0.0, (a, b) => a + (double.tryParse(b.amount ?? '0') ?? 0.0));
    double sumQty = _allRows.fold(0.0, (a, b) => a + (b.quantity ?? 0.0));

    return _CancelKotRow(
      restaurant: "Total", kotId: "", itemNames: "", quantity: sumQty,
      amount: sumAmount.toString(), status: "", notes: "", tableNumber: "",
      orderDate: "", orderTime: "", cancelTime: "", formatter: _format3,
    );
  }
  ...
  Future<void> _exportExcel() async {
    ...
    if (kIsWeb) {
      ...
      final String path = '${Directory.current.path}/CancelKotReport.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("All Cancel KOT Report", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("All Cancel KOT", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(double screenWidth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: const Text("Cancelled KOT Logs", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))),
      Expanded(
        child: Scrollbar(
...
class _CancelKotRow {
  final String restaurant, kotId, itemNames, status, tableNumber, orderDate, orderTime;
  final String? notes, cancelTime, amount;
  final double? quantity;
  final String Function(dynamic) formatter;

  _CancelKotRow({
    required this.restaurant, required this.kotId, required this.itemNames, required this.quantity,
    required this.amount, required this.status, this.notes, required this.tableNumber,
    required this.orderDate, required this.orderTime, this.cancelTime, required this.formatter
  });

  factory _CancelKotRow.fromReport({required CancelKotReport report, required String restaurant, required String Function(dynamic) formatter}) {
    return _CancelKotRow(
        restaurant: restaurant, kotId: report.kotId, itemNames: report.itemNames, quantity: report.quantity,
        amount: report.amount, status: report.status, notes: report.notes, tableNumber: report.tableNumber,
        orderDate: report.orderDate, orderTime: report.orderTime, cancelTime: report.cancelTime, formatter: formatter
    );
  }

  dynamic getField(String k) {
    switch (k) {
      case 'restaurant': return restaurant; case 'kotId': return kotId; case 'itemNames': return itemNames;
      case 'quantity': return formatter(quantity); case 'amount': return formatter(amount); case 'status': return status;
      case 'notes': return notes ?? ''; case 'tableNumber': return tableNumber; case 'orderDate': return orderDate;
      case 'orderTime': return orderTime; case 'cancelTime': return cancelTime ?? ''; default: return '';
    }
  }
}
