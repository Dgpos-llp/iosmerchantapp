...
class AllComplimentItemReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllComplimentItemReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllComplimentItemReportPage> createState() => _AllComplimentItemReportPageState();
}

class _AllComplimentItemReportPageState extends State<AllComplimentItemReportPage> {
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<ComplimentItemReport>> dbToComplimentItem =
    await UserData.fetchComplimentItemForDbs(config, dbList, startDate, endDate);

    for (final dbKey in dbToComplimentItem.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToComplimentItem[dbKey]!) {
        _allRows.add(_ComplimentItemRow(
          restaurant: brand,
          report: report,
          formatter: _format3,
        ));
  ...
  Future<void> _exportExcel() async {
    try {
      ...
      if (kIsWeb) {
        web_exporter.saveFileWeb(fileBytes!, 'ComplimentItemReport.xlsx');
        if (mounted) {
        ...
        final String path = '${directory.path}/ComplimentItemReport_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Item Complimentary", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: const Text("Complimentary Items", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
        ),
        Expanded(
          child: Scrollbar(
...
class _ComplimentItemRow {
  final String restaurant;
  final ComplimentItemReport report;
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'rate': return report.rate;
      case 'quantity': return report.quantity;
      case 'amount': return report.amount;
      case 'compRemark': return report.compRemark;
