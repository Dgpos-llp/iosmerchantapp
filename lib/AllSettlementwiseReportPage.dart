...
class _AllSettlementwiseReportPageState extends State<AllSettlementwiseReportPage> {
  ...
  final List<_Col> _allColumns = [
    ...
    const _Col('Settlement Mode', 'settlementModeName'),
    const _Col('Gross Amount', 'grossAmount'),
    const _Col('Number of Bills', 'numberOfBills'),
    const _Col('Percent To Gross', 'percentToGross'),
  ];
  ...
  Future<void> _fetchData() async {
    ...
    for (final dbKey in dbToSettlementwise.keys) {
      final brandName = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToSettlementwise[dbKey]!) {
  ...
  _SettlementwiseRow get totalRow {
    double sum(String Function(_SettlementwiseRow) getter) =>
    ...
    return _SettlementwiseRow(
      ...
      billDate: "",
      settlementModeName: "",
      grossAmount: sum((r) => r.grossAmount).toStringAsFixed(3),
  ...
  Future<void> _exportExcel() async {
    ...
    sheet.cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: rowNum))
      ..value = "Settlementwise Sales Report"
      ..cellStyle = boldStyle;
    rowNum += 2;
    ...
    if (kIsWeb) {
      web_exporter.saveFileWeb(fileBytes!, 'SettlementwiseReport.xlsx');
    } else {
      final String path = '${Directory.current.path}/SettlementwiseReport.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("Settlementwise Sales Report", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Settlementwise Sales", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(double screenWidth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: const Text("Settlement Details", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))),
      Expanded(
        child: Scrollbar(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return Container(
                        ...
                        child: Row(children: _visibleColumns.map((col) {
                          bool isNumeric = ['grossAmount', 'numberOfBills', 'percentToGross'].contains(col.key);
                          return Container(
                            width: colWidth, height: 48, alignment: isNumeric ? Alignment.centerRight : Alignment.centerLeft, padding: const EdgeInsets.symmetric(horizontal: 12),
    ...
    return Container(
      ...
      child: Row(
          children: _visibleColumns.map((col) {
            // Added numeric check for header alignment
            bool isNumeric =
            ['grossAmount', 'numberOfBills', 'percentToGross'].contains(col.key);
            return Container(
                width: w,
                height: h,
                // Changed from centerLeft to conditional alignment
...
class _SettlementwiseRow {
  final String restaurant, billDate, settlementModeName, grossAmount, numberOfBills, percentToGross;
  _SettlementwiseRow({required this.restaurant, required this.billDate, required this.settlementModeName, required this.grossAmount, required this.numberOfBills, required this.percentToGross});
  ...
  factory _SettlementwiseRow.fromReport({required SettlementwiseReport report, required String restaurant}) {
    ...
    return _SettlementwiseRow(restaurant: restaurant, billDate: report.billDate, settlementModeName: report.settlementModeName, grossAmount: f3(report.grossAmount), numberOfBills: report.numberOfBills.toString(), percentToGross: f3(report.percentToGross));
  }

  dynamic getField(String k) {
    switch (k) {
      case 'restaurant': return restaurant; case 'billDate': return billDate; case 'settlementModeName': return settlementModeName; case 'grossAmount': return grossAmount; case 'numberOfBills': return numberOfBills; case 'percentToGross': return percentToGross; default: return '';
    }
  }
}
