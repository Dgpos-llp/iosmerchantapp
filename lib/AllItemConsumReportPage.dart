...
class _AllItemConsumReportPageState extends State<AllItemConsumReportPage> {
  ...
  final List<String> _numericKeys = [
    'saleQty', 'complimentaryQty', 'noChargeQty', 'totalQty',
    'totalAmount', 'discountPercent', 'discountAmount', 'amountAfterDiscount',
    'homeDeliverySaleQty', 'dineInSaleQty', 'takeAwaySaleQty', 'onlineSaleQty', 'counterSaleQty'
  ];

  final List<_Col> _allColumns = [
    const _Col('Restaurant', 'restaurant'),
    const _Col('Product', 'productName'),
    const _Col('Code', 'productCode'),
    const _Col('Category', 'categoryName'),
    const _Col('Sale', 'saleQty'),
    const _Col('Comp', 'complimentaryQty'),
    const _Col('No Charge', 'noChargeQty'),
    const _Col('Total Qty', 'totalQty'),
    const _Col('Amount', 'totalAmount'),
    const _Col('Disc %', 'discountPercent'),
    const _Col('Disc Amt', 'discountAmount'),
    const _Col('After Disc', 'amountAfterDiscount'),
    const _Col('HD Sale', 'homeDeliverySaleQty'),
    const _Col('Dine-In', 'dineInSaleQty'),
    const _Col('Take Away', 'takeAwaySaleQty'),
    const _Col('Online', 'onlineSaleQty'),
    const _Col('Counter', 'counterSaleQty'),
  ];
  ...
  Future<void> _fetchData() async {
    ...
    for (final dbKey in dbToItemConsum.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToItemConsum[dbKey]!) {
        _allRows.add(_ItemConsumRow(restaurant: brand, report: report));
      }
    }
  ...
  Future<void> _exportExcel() async {
    try {
      ...
      if (kIsWeb) {
        web_exporter.saveFileWeb(fileBytes!, 'ItemConsumReport.xlsx');
        if (mounted) {
        ...
        final String path = '${directory.path}/ItemConsumReport_${DateTime.now().millisecondsSinceEpoch}.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          centerTitle: true,
          title: const Text("Item Consumption", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Item Consumption", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildTable(bool isMobile) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
          ...
          child: const Text("Consumption Details", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))
      ),
      Expanded(
        child: Scrollbar(
