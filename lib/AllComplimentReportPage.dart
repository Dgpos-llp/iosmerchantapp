...
class _AllComplimentReportPageState extends State<AllComplimentReportPage> {
  ...
  Future<void> _fetchData() async {
    ...
    if (selectedDbKey == null || selectedDbKey == "All") {
      final List<ComplimentReport> allItems = dbToCompliment.values.expand((x) => x).toList();
      for (final item in allItems) {
        _allRows.add(_ComplimentRow(
          restaurant: "ALL",
          report: item,
          formatter: _format3,
        ));
      ...
      for (final list in dbToCompliment.values) {
  ...
  _ComplimentRow get totalRow {
    double sum(String Function(_ComplimentRow) getter) {
    ...
    return _ComplimentRow(
      ...
      report: ComplimentReport(
        billNo: "",
        billDate: "",
        billAmount: sum((r) => r.report.billAmount).toStringAsFixed(3),
        remark: "",
        waiter: "",
      ),
      formatter: _format3,
  ...
  Future<void> _exportExcel() async {
    ...
    reportCell.value = "All Compliment Report";
    ...
    if (kIsWeb) {
      ...
      final anchor = html.AnchorElement(href: url)
        ..setAttribute('download', 'AllComplimentReport.xlsx')
        ..click();
      ...
      final String path = '${Directory.current.path}/AllComplimentReport.xlsx';
