...
class AllAuditItemReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllAuditItemReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllAuditItemReportPage> createState() => _AllAuditItemReportPageState();
}

class _AllAuditItemReportPageState extends State<AllAuditItemReportPage> {
  ...
  final List<_Col> _allColumns = [
    ...
    const _Col('Amt', 'totalPrice'),
  ];
  late List<_Col> _visibleColumns;
  List<_AuditItemRow> _allRows = [];
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<AuditItemReport>> dbToAuditItem =
    await UserData.fetchAuditItemForDbs(config, dbList, startDate, endDate);

    for (final dbKey in dbToAuditItem.keys) {
      final brand = widget.dbToBrandMap[dbKey] ?? dbKey;
      for (final report in dbToAuditItem[dbKey]!) {
        _allRows.add(_AuditItemRow(
          restaurant: brand,
          report: report,
          formatter: _format3,
        ));
        ...
        // Calculate totals
        totalQuantity += double.tryParse(report.quantity) ?? 0.0;
        totalAmount += double.tryParse(report.totalPrice) ?? 0.0;
      }
    }
  ...
  _AuditItemRow get totalRow {
    return _AuditItemRow(
      restaurant: "Total",
      report: AuditItemReport(
        billNo: "",
        formName: "",
        createdDate: "",
        userCreated: "",
        remarks: "",
        productName: "",
        quantity: _format3(totalQuantity),
        totalPrice: _format3(totalAmount),
      ),
      formatter: _format3,
    );
  }
  ...
  Future<void> _exportExcel() async {
    try {
      ...
      for (int i = 0; i < _visibleColumns.length; i++) {
        ...
        else if (colKey == 'quantity') value = _format3(totalQuantity);
        else if (colKey == 'totalPrice') value = _format3(totalAmount);
      ...
      if (kIsWeb) {
        web_exporter.saveFileWeb(fileBytes!, 'AuditItemReport.xlsx');
        if (mounted) {
        ...
        final String path = '${directory.path}/AuditItemReport_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Widget _buildBreadcrumb() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Text("Audit Item", style: TextStyle(color: Color(0xFF4154F1), fontWeight: FontWeight.w600, fontSize: 13)),
      ]),
    );
  }
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: const Text("Audit Item Details", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
        ),
        Expanded(
          child: Scrollbar(
    ...
    return Container(
      ...
      child: Row(
        children: _visibleColumns.map((col) {
          ...
          else if (col.key == 'quantity') text = _format3(totalQuantity);
          else if (col.key == 'totalPrice') text = _format3(totalAmount);
...
class _AuditItemRow {
  ...
  dynamic getField(String key) {
    ...
    switch (key) {
      ...
      case 'totalPrice': return report.totalPrice;
      default: return '';
    }
  }
}
