...
class _AllTimeAuditReportPageState extends State<AllTimeAuditReportPage> {
  ...
  _TimeAuditRow get totalRow {
    ...
    return _TimeAuditRow(
      restaurant: "Total", billNo: "", tableNo: "", kotTime: "", billDate: "",
      billTime: "", settleDate: "", settleTime: "", userCreated: "",
      userEdited: "", remarks: "",
  ...
  Future<void> _exportExcel() async {
    ...
    if (kIsWeb) {
      ...
      final String path = '${Directory.current.path}/TimeAudit.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("Time Audit Report", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF2C3E50))),
  ...
  Widget _buildTable(double screenWidth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: const Text("Audit Logs", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50)))),
      Expanded(
        child: Scrollbar(
...
class _TimeAuditRow {
  ...
  _TimeAuditRow({required this.restaurant, required this.billNo, required this.tableNo, required this.kotTime, required this.billDate, required this.billTime, required this.settleDate, required this.settleTime, required this.userCreated, required this.userEdited, this.remarks, required this.timeDifference, required this.billAmount, required this.settlementMode});
  ...
  factory _TimeAuditRow.fromReport({required TimeAuditReport report, required String restaurant}) {
    ...
    return _TimeAuditRow(restaurant: restaurant, billNo: report.billNo, tableNo: report.tableNo, kotTime: report.kotTime, billDate: report.billDate, billTime: report.billTime, settleDate: report.settleDate, settleTime: report.settleTime, userCreated: report.userCreated, userEdited: report.userEdited, remarks: report.remarks ?? "", timeDifference: report.timeDifference, billAmount: f3(report.billAmount), settlementMode: report.settlementMode);
  }

  dynamic getField(String k) {
    switch (k) {
      case 'restaurant': return restaurant; case 'billNo': return billNo; case 'tableNo': return tableNo; case 'kotTime': return kotTime; case 'billDate': return billDate; case 'billTime': return billTime; case 'settleDate': return settleDate; case 'settleTime': return settleTime; case 'userCreated': return userCreated; case 'userEdited': return userEdited; case 'remarks': return remarks; case 'timeDifference': return timeDifference; case 'billAmount': return billAmount; case 'settlementMode': return settlementMode; default: return '';
    }
  }
}
